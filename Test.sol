...
library Address {
     ...
     * https://solidity.readthedocs.io/en/v0.5.11/security-considerations.
     * html#use-the-checks-effects-interactions-pattern[checks-effects-interactions pattern].
     */
...
contract Nekocoin is Context, IERC20, Ownable {
    ...
    uint256 private _totalSupply = 10**9*10**9; // 1 billion of neko issued in Genesis.
    ...
    uint256 private thresholdAmount = 10**6*10**9; // The threshold amount of tokens the contract hold to launch a swapAndLiquify event.
    uint256 private reflectRate = 1; // The rate of static rewarding and burnning reflected from transferred tokens.
    uint256 private liquifyRate = 1; // 1% of transferred tokens to be added as liquidity.
    
    mapping(address => uint256) private balances; // The tokens owned by the account without rewards.
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool) {
        ...
        //require(amount <= allowances[sender][_msgSender()]);
        _transfer(sender, recipient, amount);
        //allowances[sender][_msgSender()] = allowances[sender][_msgSender()].sub(amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, 
        "ERC20: transfer amount exceeds allowance"));
        return true;
    }

    function increaseAllowance(address spender, uint256 addedVal) public virtual returns(bool) {
         _allowances[_msgSender()][spender] = _allowances[_msgSender()][spender].add(addedVal);
         return true;
     }
     
    function decreaseAllowance(address spender, uint256 subtractedVal) public virtual returns(bool) {
         require(subtractedVal<=_allowances[_msgSender()][spender], "ERC20: Insufficient allowance to be subtracted");
         _allowances[_msgSender()][spender] = _allowances[_msgSender()][spender].sub(subtractedVal);
         return true;
     }
    ...
    function setSwapAndLiqEnabled(bool enabled) public onlyOwner {
    ...
    function getValues(uint256 amount) private view returns(uint256, uint256, uint256, uint256) {
        (uint256 taxForReflect, uint256 taxForLiquify) = _getTaxVals(amount);
    ...
    function _getTaxVals(uint256 amount) private view returns(uint256, uint256) {
    ...
    function takeLiquidity(uint256 taxForLiq) private {
        uint256 currentRate =  getExRate();
        uint256 implicitLiq = taxForLiq.div(currentRate);
        balances[address(this)] = balances[address(this)].add(implicitLiq);
    }
    ...
    function tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 implicitAmount = amount.add(taxForReflect).add(taxForLiquify).div(getExRate()); // back to no refelction state
        balances[sender] = balances[sender].sub(implicitAmount);
        balances[recipient] = balances[recipient].add(implicitAmount);
        _totalReward.add(reward);
        _totalSupply.sub(burn); 
        _burned.add(burn);
        takeLiquidity(taxForLiquify);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // Swap tokens for BNB.
        swapTokensForBNB(halfForBNB); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
