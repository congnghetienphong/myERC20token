...
contract Nekocoin is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private constant _name = 'Nekoa';
    string private constant _symbol= 'NEKOa';
    ...
    uint256 private constant thresholdAmount = 10**4*10**uint256(_decimals); // The threshold amount of tokens the contract hold to launch a swapAndLiquify event.
    ...
    constructor() public {
        _totalReward = 1;
        _burned = 1; 
        balances[_msgSender()] = _totalSupply-2;
    ...
    function balanceOf(address account) public override view returns(uint256) {
        ...
        return balances[account].mul(exRate()+1).div(exRate());
    }
    ...
    function getTaxVals(uint256 amount) private view returns(uint256, uint256, uint256) {
        uint256 transferredAmount = amount.mul(exRate()).div(exRate()+1); // back to no refelction state
    ...
    function getReflectVals(uint256 taxForRef, uint256 burnedRatio) private pure returns(uint256, uint256) {
        uint256 reward = taxForRef.div(burnedRatio); // Multiplied by burned percentage.
        uint256 burn = taxForRef - reward;
        return(reward, burn);
    }
    
    // The burend ratio from total supply determines the proportion of reward and burn in tax.
    function burnedRatio() public view returns(uint256) {
        return _totalSupply.div(_burned);
    }
    
    // Get the exchange rate between the no-reward balance and the actual balance
    function exRate() public view returns(uint256) {
        return _totalSupply.div(_totalReward);
    }
